# STEP 5B: Undo last item (type 'undo' at item prompt)
...
print("Type 'done' to finish. Type 'undo' to remove the last item.\n")
...
while True:
    ...
    choice = input("\nChoose a product number (1-5), 'undo', or 'done': ").strip().lower()
    ...
    if choice == "undo":
        if not purchases:
            print("There's nothing to undo yet.\n")
            continue

        removed = purchases.pop()
        total -= removed["line_total"]

        print(f"Removed: {removed['name']} x{removed['quantity']} (-£{removed['line_total']:.2f})")
    ...
    try:
        ...
        print("You need to use valid numbers here (1-5), or type 'undo'/'done'. Try again.\n")
        continue
